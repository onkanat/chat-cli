...
- Arayüzyü root (top-level) slash komutları şeklinde genişletir:

| Komut | Açıklama |
|---|---|
| `/wiki pin <sayfa>` | Bir sayfayı sohbet bağlamına sabitler |
| `/wiki unpin` | Sabitlenmiş sayfayı kaldırır |
| `/wiki search <sorgu>` | Wiki içerisinde semantik arama yapar, numaralandırılmış tablo döner |
| `/wiki open <no>` | Arama sonucundaki dosyayı context'e dahil eder |
| `/wiki add <dosya> <başlık>` | Multi-line Rich-Prompt editörüyle yeni wiki sayfası oluşturur |
| `/wiki ingest <metin>` | Ham metin notunu `raw/` altına kaydeder; omni-daemon arka planda wiki'ye entegre eder |
| `/wiki ingest-session` | Aktif sohbeti `raw/session_<ts>.md` olarak kaydeder; omni-daemon watchdog ile wiki'ye özetler |
| `/wiki show`, `/wiki edit`, `/wiki delete`, `/wiki list` | Sayfa okuma, düzenleme, silme ve listeleme |
| `/wiki help` | Tüm Wiki komut setinin yardım tablosunu açar |

> [!NOTE]
> `/wiki ingest` = tek ham not → wiki. `/wiki ingest-session` = tüm aktif sohbet özeti → wiki.
> Eğitim verisi üretmek için `/distill` kullanın; bu iki pipeline birbirinden bağımsızdır.

#### Distill Plugin (⚗️ Conversation Distillation)

- `plugins/distill_plugin.py` — Aktif sohbet geçmişini **Alpaca JSONL eğitim verisine** dönüştürür.
- `LLMwiki/scripts/distill_chats.py` üzerinden çalışır; tüm işlem arka planda (daemon thread) yürütülür.
- Çıktılar `LLMwiki/datasets/dataset_<topic>.jsonl` ve `wiki/distilled/report_<topic>.md` dosyalarına yazılır.
- Thinking modeller (qwen3, deepseek-r1) için otomatik `<think>` blok stripping uygulanır.

| Komut | Açıklama |
|---|---|
| `/distill` | Aktif `chat_history.json`'u distill eder (async, bloke etmez) |
| `/distill session` | (alias) Aynı işlem |
| `/distill status` | Son üretilen dataset dosyalarını tablo olarak listeler |
| `/distill help` | Pipeline ayırımı ve komut açıklaması |

> [!IMPORTANT]
> **Pipeline Ayırımı**: `/distill` → eğitim verisi (JSONL), `/wiki ingest-session` → wiki bilgisi (Markdown).
> İkisi birbirini tamamlar ama karıştırılmamalıdır.
...
Bu proje artık resmi Ollama Python kütüphanesini (`ollama>=0.6.1`) kullanır. Eski `ollama-python` bağımlılığı kaldırıldı. Entegrasyon, küçük bir `lib/ollama_wrapper.py` modülü üzerinden yapılır ve şu yetenekleri sağlar:
...
# Python içinden
python -c "from lib import ollama_wrapper as ow; print(ow.list_models())"
```
...
```json
{
  "enabled_plugins": ["persona_selector", "file_plugin", "wiki_plugin", "distill_plugin"],
  "plugin_settings": {
    "persona_selector": {
      "system_prompts_path": "/path/to/system_prompts"
    },
    "wiki_plugin": {
      "omni_daemon_url": "http://localhost:8000",
      "wiki_path": "/path/to/LLMwiki",
      "auto_context": false,
      "context_top_k": 3,
      "request_timeout": 30
    },
    "distill_plugin": {
      "llmwiki_path": "/path/to/LLMwiki"
    }
  },
  "auto_load": true
}
...
Projede kapsamlı bir test seti bulunmaktadır (7 test dosyası). Çalıştırmak için:
...
```text
.
├── main.py                    # CLI bootstrap (225 lines)
├── lib/                       # Core utilities (2,306 lines)
│   ├── history.py            # History processing & context
...
│   └── shell_output.py      # Shell output handling
├── ui/                        # Terminal UI (941 lines)
│   ├── console.py            # Rich console utilities
│   ├── inputs.py             # User input components
│   ├── renderers.py          # Output formatting
│   ├── settings_menus.py     # Settings interface
│   └── stream_display.py    # Streaming display
├── services/                  # Business logic (89 lines)
│   ├── server_profiles.py    # Server configuration
│   ├── settings_service.py   # Settings management
│   └── models_service.py     # Model operations
├── commands/                  # CLI handlers (49 lines)
│   ├── chat.py               # Chat command
│   ├── list_models.py        # Model listing
│   └── save_history.py      # History saving
├── repl/                      # REPL implementation (634 lines)
│   └── loop.py              # Interactive loop & slash commands
├── plugins/                   # Plugin system
│   ├── wiki_plugin.py        # Wiki management (Omni-Context V2)
│   ├── distill_plugin.py     # Conversation distillation pipeline
│   ├── persona_selector.py   # Persona management
│   ├── file_plugin.py        # File operations
│   ├── example_plugin.py     # Example plugin template
│   └── system_prompts/      # Persona templates
├── analytics/                 # Analytics system
│   ├── manager.py            # Analytics manager
│   └── views.py              # Analytics views
├── tests/                     # Test suite (7 files)
│   ├── test_history_processing.py
│   ├── test_dual_history.py
│   ├── test_model_profiles.py
│   ├── test_persona_selector_plugin.py
│   ├── test_session_management.py
│   ├── test_smart_shell.py
│   └── test_system_compression.py
├── docs/                      # Documentation
