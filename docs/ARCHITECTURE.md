...
## 🧠 Knowledge Pipeline Mimarisi

Bu iki pipeline birbirini tamamlar ama görev ayrımları nettir:

### Pipeline 1 — Distillation (Eğitim Verisi Üretimi)

```
chat_history.json
    │
    └─► /distill (distill_plugin.py)
            │  daemon thread (bloke etmez)
            └─► LLMwiki/scripts/distill_chats.py
                    ├─► Segmentasyon (auto chunk: token bazlı)
                    ├─► Ollama /api/generate (format:json YOK — thinking model uyumu)
                    ├─► <think> blok stripping + JSON regex fallback
                    ├─► Pydantic DistillationResult doğrulaması
                    ├─► wiki/distilled/report_<topic>.md  (bilgi raporu)
                    └─► datasets/dataset_<topic>.jsonl   (Alpaca fine-tune verisi)
```
...
| Komut | Açıklama |
|---|---|
| `/distill` | chat_history.json'u async distill eder |
| `/distill status` | datasets/ klasörünü tablo olarak listeler |
| `/distill help` | Pipeline açıklaması |

**Konfigürasyon (`LLMwiki/wiki_config.json`):**

```json
{
  "chat_history_path": "/path/to/chat_history.json",
  "distill_chunk_size": "auto",
  "distill_dedup": true,
  "llm_timeout_seconds": 300,
  "distilled_output_path": "wiki/distilled",
  "dataset_output_path": "datasets"
}
```

**Konfigürasyon (`plugin_config.json`):**
...
### Pipeline 2 — Wiki Ingest (Bilgi Tabanı Güncelleme)

```
❶ Tek not:
/wiki ingest <metin>
    └─► raw/ingest_<ts>.md  ────► omni-daemon watchdog (~1s algılar)
                                        └─► LLM synthesis
                                        └─► wiki/sources/<slug>.md
                                        └─► wiki/index.md güncellenir
                                        └─► log.md'ye kayıt

❷ Sohbet özeti:
/wiki ingest-session
    └─► raw/session_<ts>.md ────► omni-daemon watchdog (~1s algılar)
                                        └─► (aynı pipeline)
```
...
| Komut | Açıklama |
|---|---|
| `/wiki ingest <metin>` | Ham not → wiki (arka planda) |
| `/wiki ingest-session` | Aktif sohbet → wiki özeti (raw/ dump stratejisi) |

**REST API (omni-daemon):**

| Endpoint | Açıklama |
|---|---|
| `POST /api/v1/wiki/ingest` | Tek dosya manuel ingest |
| `GET /api/v1/wiki/status` | Wiki sayfa sayısı, son log girişi |
| `POST /api/v1/wiki/search` | Semantik arama |

---

### Pipeline Sorumluluk Matrisi

| | **Distill** | **Wiki Ingest** |
|---|---|---|
| Girdi | `chat_history.json` | Metin / dosya |
| Çıktı | `datasets/dataset_*.jsonl` | `wiki/sources/*.md` |
| Amaç | Modeli eğit (fine-tune) | Kalıcı bilgi tabanı |
| LLM Prompt | Distillation Engine | WikiUpdaterService |
| Tetikleyici | `/distill` | `/wiki ingest[-session]` |
